...
<html>
    <head>
        <title><?php echo ucfirst($filter); ?></title>
    ...
    <?php
        $arr['filter'] = $filter;
        $sql = "select * from property where status = :filter order by timestamp desc";
                ...
                if($statement)
                {
                    ...
                    if($check)
                    {
                        ...
                        if(count($data)> 0)
                        {
                            ...
                            <h1 style="font-family: 'Jua';">Properties <?php echo $filter; ?></h1>
                            <div class="card-deck">
                            ...
                            foreach ($data as $rows)
                            {
                                    ...
                                    if($check1)
                                    {
                                        ...
                                        {   
                                            foreach ($pics as $row)
                                                {
                                                    ?>
                                                    <img src="user/pictures/<?php echo $row['image_name']; ?>" style="width:100%">
                                                <?php
                                                }
                                        }
                                    }
                                ...
                                <a href="propertyDisplay.php?property_id=<?php echo $rows['property_id'];?>&filter=<?php echo $filter;?>" class="btn btn-block stretched-link">Details</a>
                            </div>
                            </div>
                                <?php
                            }
